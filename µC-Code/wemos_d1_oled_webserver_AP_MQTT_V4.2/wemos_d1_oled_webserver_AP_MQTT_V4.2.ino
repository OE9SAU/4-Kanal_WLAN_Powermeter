...
void handleRoot() {
  ...
  String html = "<!DOCTYPE html><html lang='de'><head><meta charset='UTF-8'>"
                ...
                "<style>body{background:#121212;color:#e0e0e0;font-family:sans-serif;padding:20px;}"
                "h1{text-align:center;font-size:2.5em;}table{margin:auto;border-collapse:collapse;}"
                "th,td{padding:10px;border-bottom:1px solid #333;}th{background:#222;}"
                "footer{margin-top:2em;text-align:center;font-size:0.9em;color:#888;}</style></head><body>"
                "<h1>OE9XVI Powersupply Monitor</h1><div class='release'>" + String(Release) + "</div>"
                "<table><thead><tr><th>Sensor</th><th>Strom (A)</th><th>Spannung (V)</th></tr></thead><tbody>"
...
void sendDiscovery() {
  ...
  struct SensorConfig {
    ...
    {"BAT Strom", "OE9XVI-PSU/BAT/Strom", "A", "current", "oe9xvi_bat_current"},
    {"BAT Spannung", "OE9XVI-PSU/BAT/Spannung", "V", "voltage", "oe9xvi_bat_voltage"},
    {"NT1 Strom", "OE9XVI-PSU/NT1/Strom", "A", "current", "oe9xvi_nt1_current"},
    {"NT1 Spannung", "OE9XVI-PSU/NT1/Spannung", "V", "voltage", "oe9xvi_nt1_voltage"},
    {"NT2 Strom", "OE9XVI-PSU/NT2/Strom", "A", "current", "oe9xvi_nt2_current"},
    {"NT2 Spannung", "OE9XVI-PSU/NT2/Spannung", "V", "voltage", "oe9xvi_nt2_voltage"},
    {"NT3 Strom", "OE9XVI-PSU/NT3/Strom", "A", "current", "oe9xvi_nt3_current"},
    {"NT3 Spannung", "OE9XVI-PSU/NT3/Spannung", "V", "voltage", "oe9xvi_nt3_voltage"},
  };

  for (auto& s : sensors) {
    doc.clear();
    doc["name"] = s.name;
    doc["state_topic"] = s.state_topic;
    doc["unit_of_measurement"] = s.unit;
    doc["device_class"] = s.device_class;
    doc["unique_id"] = s.unique_id;
    doc["availability_topic"] = "OE9XVI-PSU/status";
    serializeJson(doc, buf);
    String topic = "homeassistant/sensor/" + String(s.unique_id) + "/config";
    mqttClient.publish(topic.c_str(), buf, true);
  }

  mqttClient.publish("OE9XVI-PSU/status", "online", true);
}
...
void setupMQTT() {
  ...
  if (strlen(mqttConfig.broker) > 0) {
    if (mqttClient.connect("PSUClient", mqttConfig.username, mqttConfig.password)) {
      ...
      sendDiscovery();  // <---- Discovery senden
    } else {
...
void publishAllPsuValues() {
  mqttClient.publish("OE9XVI-PSU/BAT/Strom", String(c00, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/BAT/Spannung", String(v0, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT1/Strom", String(c11, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT1/Spannung", String(v1, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT2/Strom", String(c22, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT2/Spannung", String(v2, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT3/Strom", String(c33, 2).c_str(), true);
  mqttClient.publish("OE9XVI-PSU/NT3/Spannung", String(v3, 2).c_str(), true);
}
...
void loop() {
  ...
  c00 = INA0.getCurrent(); v0 = INA0.getBusVoltage();
  c11 = INA1.getCurrent(); v1 = INA1.getBusVoltage();
  c22 = INA2.getCurrent(); v2 = INA2.getBusVoltage();
  c33 = INA3.getCurrent(); v3 = INA3.getBusVoltage();
