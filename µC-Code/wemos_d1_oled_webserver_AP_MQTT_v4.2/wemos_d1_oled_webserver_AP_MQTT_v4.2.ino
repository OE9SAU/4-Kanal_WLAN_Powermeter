...
// LED-Status
const int ledPin = LED_BUILTIN;  // Meist GPIO2 (invertiert: LOW = AN)
bool ledState = false;
unsigned long lastLedToggle = 0;
unsigned long ledInterval = 200;  // Initial: schnell blinken

// ========== Versionsinfo ==========
const char* Release = "V4.2 by OE9SAU";
...
struct MqttConfig {
  ...
  char broker[64] = "";  // Standardwert oder leer lassen
  int port = 1883;
  char username[32] = "";
  char password[32] = "";
  char topic[64] = "";
...
void setup() {
  ...
  digitalWrite(ledPin, LOW); // LED aus (invertiert)
...
// === LED-Status abhängig vom WLAN-Verbindungsstatus ===
  if (WiFi.status() == WL_CONNECTED) {
    ledInterval = 1000;  // langsam blinken bei Verbindung
  } else {
    ledInterval = 200;   // schnell blinken bei Trennung
  }

  if (millis() - lastLedToggle > ledInterval) {
    lastLedToggle = millis();
    ledState = !ledState;
    digitalWrite(ledPin, ledState ? LOW : HIGH);  // Invertierte Logik
  }
